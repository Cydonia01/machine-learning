...
ale = ALEInterface()
ale.loadROM("kung_fu_master.bin")
...
class Network(nn.Module):
    def __init__(self, action_size):
        ...
        self.fc2a = nn.Linear(128, action_size) # For policy
        self.fc2s = nn.Linear(128, 1) # For value
...
class Agent():
    ...
    def act(self, state):
        if state.ndim == 3:
            state = [state]
        state = state = torch.from_numpy(np.array(state)).float().to(self.device)
...
class EnvBatch:
    ...
    def step(self, actions):
        next_states, rewards, dones, infos, _ = map(np.array, zip(*[env.step(action) for env, action in zip(self.envs, actions)]))
...
with tqdm.trange(0, 4001) as progress_bar:
...
torch.save(agent.network.state_dict(), 'final_weights.pth')

# agent.network.load_state_dict(torch.load('final_weights.pth', map_location=agent.device))
...
show_video_of_model(agent, 'KungFuMaster-v4')
